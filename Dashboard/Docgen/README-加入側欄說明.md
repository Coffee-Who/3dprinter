...
Dashboard/
 ...
 ├ template.pptx         ← 新增：PPT 底稿範本
 └ template.docm         ← 新增：Word 確認單底稿（含浮水印與巨集）
```
...
- **操作流程**：預設開在 Word 確認單（主要輸入）→ 填完按「⇄ 套用到 PPT」，客戶名稱、廠牌＋型號（自動帶精度標準/公差）、列印時間、完成日期、9 格照片與 CAD/誤差（自動回推量測值）全部帶入 PPT，再補 PPT 專屬欄位即可產出兩份。
...
- **Word 確認單**以原版 .docm 為底稿輸出 .docm：浮水印、頁首 LOGO、框線、巨集完整保留；僅填入紅圈欄位、V 勾與 9 格照片/CAD/誤差。
- **跨電腦**：設定檔存在各自瀏覽器；要帶到別台電腦用「匯出 JSON → 匯入 JSON」。
- 檔名自動：`日期_客戶公司全名_材料特性與列印尺寸確認.pptx`、`日期_客戶_3D列印服務確認單.docx`。

## 四、注意事項

- `template.pptx`、`template.docm` 都要跟 `docgen-full.html` 同資料夾，否則產生時會提示讀不到範本。
