...
def build_printer_map(printers: list) -> dict:
    """
    回傳 {serial: alias_or_name} 對照表。
    Formlabs API 的 prints.printer 欄位是 serial，
    需要對照這個 map 才能顯示真實機器名稱。
    """
    m = {}
    for p in printers:
        serial = p.get("serial", "")
...
def list_prints(token, date_gt=None, date_lt=None,
    ...
    resp = requests.get(f"{BASE}/prints/", headers=_h(token),
                        params=params, timeout=25)
    resp.raise_for_status()
    return resp.json()


def list_all_prints(token, **kwargs) -> list:
    """Auto-paginate — returns all matching print records."""
    results, page = [], 1
    while True:
...
def normalise_prints(prints: list, printer_map: dict) -> list:
    """
    把每筆列印紀錄的 printer(serial) 換成 printer_name(alias)，
    並把 material_name 補齊。
    """
    out = []
    for p in prints:
        p = dict(p)  # shallow copy，避免污染原始資料
        serial = p.get("printer", "")
        # 機器名稱：優先用 alias，fallback 用 serial
        p["printer_name"] = printer_map.get(serial, serial) if serial else "未知"

        # 材料：material_name 優先，fallback material
