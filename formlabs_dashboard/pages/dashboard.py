...
def render():
    ...
    # ── DEBUG expander（Live 模式下顯示）────────────────────────────────────
    if not st.session_state.use_demo:
        with st.expander("🔍 API 原始資料檢查（若機器/材料顯示不全請展開查看）",
                          expanded=False):
            st.markdown("##### 📡 API 回傳印表機清單")
            printer_debug = []
            for p in printers:
                ps = p.get("printer_status") or {}
                printer_debug.append({
                    "serial":          p.get("serial",""),
                    "alias":           p.get("alias",""),
            ...
            st.dataframe(pd.DataFrame(printer_debug), use_container_width=True)

            st.markdown(f"**Serial → Alias 對照表**（共 {len(printer_map)} 台）")
            st.json(printer_map)

            if prints:
                # 找出 prints 中不重複的 printer 值
                all_printer_vals = list({pr.get("printer","") for pr in prints})
                st.markdown(f"##### 📋 列印紀錄中的 printer 欄位（共 {len(all_printer_vals)} 種值）")
                st.write(sorted(all_printer_vals))

                all_mat_vals = list({pr.get("material_name","") or pr.get("material","") for pr in prints})
                st.markdown(f"##### 🧴 列印紀錄中的材料欄位（共 {len(all_mat_vals)} 種）")
                st.write(sorted(all_mat_vals))

                st.markdown("##### 前 5 筆列印紀錄原始欄位")
                for i, pr in enumerate(prints[:5]):
                    with st.container():
                ...
                st.warning("⚠️ 目前日期範圍內無列印紀錄，請確認日期區間是否正確")
